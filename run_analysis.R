...
## Change the names for each variables of the sets
...
## Create a table with the average of each variable for each activity and subject
## Apply the mean function to the subset of variables (.SD) for each ID and Activity
cleanDataTableAverages <- cleanDataTable[, lapply(.SD, mean), by = 'ID,Activity']

## Write the tidy data set in a file
write.table(cleanDataTableAverages, file = "Tidy_Dat_Set.txt", row.names = FALSE)
